...
class Astar:
    ...
    def search(self) -> None:
        hp.heappush(self.hnode, (0, self.start))
        while len(self.nodes) > 0:
            ...
            for n in neb:
                ...
                heu = self.__heuristicVal(n)
                total_dist = temp_dist + heu
                if total_dist < self.distance[n]:
                    self.distance[n] = total_dist
                    self.best_previous[n] = u
                    temp_tuple = (total_dist, n)
                    hp.heappush(self.hnode, temp_tuple)
    ...
    def __minDistance(self) -> int:
        ...
        index = None
        while flag:
            u_tuple = hp.heappop(self.hnode)
            if u_tuple[1] in self.nodes:
                index = u_tuple[1]
                flag = False
    ...
    def __heuristicVal(self, u: int) -> int:
        ur, uc = u // self.c, u % self.c
        gr, gc = self.goal_coord[0], self.goal_coord[1]
        return int(math.sqrt((ur - gr) ** 2 + (uc - gc) ** 2))
